...
| 5주차 | 5/6 \~ 5/12  | 체크포인트 및 클리어 조건 구현, 배경 스크롤 |  100% |
| 6주차 | 5/13 \~ 5/19 | 적 패턴 및 피격 처리, 사운드 추가      |  100%   |
| 7주차 | 5/20 \~ 5/26 | 맵 에디터 구현 -> Tiled맵 에디터 사용, 맵 로더 구현  |  100%   |
| 8주차 | 5/27 \~ 6/2  | 디버깅, 버그 수정, 최종 완성         |  100%   |


## 주별 commit 수
![image](https://github.com/user-attachments/assets/f16dc59c-1136-47fe-b263-c9fda458163b)
| 주차 | commit 수 |
|------|---------|
| 0주차 | 6회     |
| 1주차 | 6회     |
...
## 변경된 목표
7주차의 맵 에디터 구현 -> Tiled맵 에디터 사용, 맵 로더 구현
...
## (앱을 스토어에 판다면) 팔기 위해 보충할 것
1. 스테이지/캐릭터/이펙트 추가
2. 화면 다중 터치 

## 결국 해결하지 못한 문제/버그
1. 화면 다중 터치를 구현하려 했지만 하지 못함
2. 맵 스크롤을 구현하는데 어려움이 있었음

## 이번 수업에서 기대한 것
단순한 예제 수준이 아닌 완성도 있는 2D 모바일 게임을 직접 구현해보는 경험을 기대했습니다

## 이번 수업에서 얻은 것
교수님의 GitHub 예제를 기반으로 한 다양한 수업 콘텐츠를 통해 게임 프로그래밍의 전반적인 흐름을 익힐 수 있었습니다

## 얻지 못한 것
여러 객체의 동시 터치를 처리하는 방법
